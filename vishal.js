...
function solution(D) {
    ...
    const occ = {'Mon': 0, 'Tue': 0, 'Wed': 0, 'Thu': 0, 'Fri': 0, 'Sat': 0, 'Sun': 0,}
    ...
    for(let i=0;i<keys.length;i++){
        ...
        occ[days[date.getDay()]]++;
    }
    
    // For checking the occurence and getting the mean value for the unmark keys
    for(let i=0;i<7;i++){
        if(occ[og[i]]===0){
            let j=i-1,k=i+1;
            let cnt1=0,cnt2=0;
...
for(var key in out2){
    console.log(key,out2[key]);
}
console.log('\n');

const out3 = solution(D3);
for(var key in out3){
    console.log(key,out3[key]);
}
console.log('\n');
